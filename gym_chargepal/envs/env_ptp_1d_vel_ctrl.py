...
from gym_chargepal.envs.config import ENVIRONMENT_PTP_1DOF_CARTESIAN_VELOCITY_CONTROL
# components
...
from gym_chargepal.controllers.controller_pos_1dof_cartesian import Position1dofCartesianController
...
class EnvironmentP2PCartesian1DVelocityCtrl(Environment):
    """ 
    Cartesian Environment with 3 dof velocity controller
    Task: point to point 
    """
    def __init__(self, **kwargs: Any):
        ...
        config: Dict[str, Any] = copy.deepcopy(
            ENVIRONMENT_PTP_1DOF_CARTESIAN_VELOCITY_CONTROL)
        config.update(config_env)
        Environment.__init__(self, config)

        # extract component hyparparameter from kwargs
